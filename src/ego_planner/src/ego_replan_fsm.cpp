...
namespace ego_planner
{
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if ((odom_pos_.array() < map_origin.array()).any() || (odom_pos_.array() > map_max.array()).any())
    {
      RCLCPP_ERROR(
          node_->get_logger(),
          "Current odom position [%.3f, %.3f, %.3f] is outside grid_map range x:[%.3f, %.3f] y:[%.3f, %.3f] z:[%.3f, %.3f]. "
          "Please adjust grid_map/map_size_* or grid_map/ground_height before sending a goal.",
          odom_pos_(0), odom_pos_(1), odom_pos_(2),
          map_origin(0), map_max(0), map_origin(1), map_max(1), map_origin(2), map_max(2));
      return;
    }
  ...
  void EGOReplanFSM::referencePathCallback(const std::shared_ptr<const nav_msgs::msg::Path> &msg)
  {
    if (msg->poses.empty())
    {
      RCLCPP_WARN(node_->get_logger(), "Received empty reference path, ignoring.");
      return;
    }
    ...
    if (planReferencePath(waypoints))
    {
      RCLCPP_INFO(node_->get_logger(), "Accepted reference path with %zu poses", msg->poses.size());
    }
  }
  ...
  bool EGOReplanFSM::planReferencePath(const std::vector<Eigen::Vector3d> &waypoints)
  {
    if (waypoints.empty())
    {
      return false;
    ...
    constexpr double step_size_t = 0.1;
    int i_end = floor(planner_manager_->global_data_.global_duration_ / step_size_t);
    vector<Eigen::Vector3d> global_traj(std::max(i_end, 1));
    for (int i = 0; i < i_end; i++)
    {
    ...
    if (i_end <= 0)
    {
      global_traj[0] = end_pt_;
    }

    if (exec_state_ == WAIT_TARGET)
    {
      changeFSMExecState(GEN_NEW_TRAJ, "PATH");
    }
    else
    {
      changeFSMExecState(REPLAN_TRAJ, "PATH");
    }
